...
if (!$procedure) {
	...
	$sql = "SELECT `ORDINAL_POSITION`,`PARAMETER_MODE`,`PARAMETER_NAME`,`DTD_IDENTIFIER` FROM `INFORMATION_SCHEMA`.`PARAMETERS` WHERE `SPECIFIC_SCHEMA` = '$database' AND `SPECIFIC_NAME` = '$procedure' ORDER BY `ORDINAL_POSITION`;";
	...
	if ($method == 'GET') {
		...
		$sql = "$input; CALL `$procedure` ($names); select $output;";
		//die($sql);
